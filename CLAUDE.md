...
- `src/lib/modulos.ts` es el catálogo único de módulos (etiqueta, ruta, icono, permiso, `cargos`, `implementado`) que consumen la barra lateral y la pantalla de inicio. `cargos` es **enfoque, no seguridad**: la sesión trae el cargo del empleado (`usuario.cargo`) para que "Mis entregas" solo aparezca al repartidor.
...
- Las pantallas de seguimiento (mis pedidos, mis entregas, tablero) se refrescan solas con `usarRefrescoPeriodico`, que solo consulta con la pestaña visible.
